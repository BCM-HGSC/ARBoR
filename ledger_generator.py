...
# HTML report will be considered invalid if any values of these fields are None.
HTML_REQUIRED_FIELDS = set([
                            SAMPLE,
                            #RPTID,#Note: allow None for HTML_RPTID_KEY field to support legacy HTML reports.
                            RPTDATE,
                            PATIENT,
])
...
# JJ_TODO: Pretty much all the reamining code in this section is from the ledger_validator module - DRY!
...
def init_rsa(privatekey_path=DEFAULT_PRIVATE_KEY_FILE, publickey_path=DEFAULT_PUBLIC_KEY_FILE):
    ''' Import RSA keys (or create new if not found) and initialize file signer. '''
...
def load_signer(privatekey):
    ''' Initialize global object SIGNER with an RSA PrivateKey object.
        If key is too small for signing or otherwise invalid, an exception is thrown. '''
    global SIGNER
    SIGNER = PKCS.new(privatekey)
    assert SIGNER.can_sign(), 'Invalid private key - Generate new keys before retrying.'
...
def cleanup_dirpath(path):
    ''' Ensure path is a directory, and append a separator ('/' for UNIX) if needed. '''
    cleanpath = os.path.normpath(path)
    if not os.path.isdir(cleanpath):
        raise Exception('Path should be a directory, inputted path was not a directory: "%s"' % path)
    cleanpath = os.path.join(cleanpath,'')
    return cleanpath

def get_filepaths(directory, filepattern, recursive=False):
    ''' Generator function for filepaths of files with names that match filepattern within directory.
        Additionally, if recursive is True, then subdirectories within directory will also be searched. '''
    if recursive:
...
def find_similar_files(filepath, excludefiles=set(), suffix_pattern=SUPPL_FILENAME_PATTERN):
    ''' Find files in the same directory with the same base name as filepath.
        Paths in excludefiles are excluded from the return.
        e.g. 'rpdir/rpt1.xml' may return [rpdir/rpt1.pdf, rpdir/rpt1-otherfile.txt] '''
    ...
    filepaths = [path for path in glob.glob(pattern) if path not in excludefiles and os.path.isfile(path)]
    return filepaths
...
#    matches = htmlsoup.findAll('table', attrs=HTML_PHI_TABLE_ID_ATTRS) #dev_note: probably shouldn't rely on a style for identifying phi table.
    ...
    # JJ_TODO: Using regexp up front is probably a performance bottleneck. Consider finding all table elements and then determining which is the correct one.
    matches = htmlsoup.find_all(text=pattern)
    tables = [match.findParent('table') for match in matches if match.findParent('table')] # If-clause is needed to exclude 'None' list elements.
    if len(tables) == 1:
        phi_table = tables[0]
    else:
        raise Exception('Unable to unequivocally determine PHI table in HTML file')
    return phi_table
...
def parse_html_report(html_report_filepath):
    ...
    return {HTML_FIELDS_MAP[field] : fields.get(field) for field in HTML_FIELDS_MAP}
...
def parse_xml_files(filepaths):
    ''' Parse data needed for ledger from XML files.
        Returns a dictionary containing initial ledger entries for valid XML files.'''
    records = list()
...
def create_pdf_rec_from_html_reports(html_report_filepaths):
    ...
    for html_path in html_report_filepaths:
        try:
            ...
            # Check required fields are valid.
            # JJ_TODO: Consider having the Required_Fields validation in create_ledger() function to operate on all records at same time.
            if None or '' in [rec[field] for field in HTML_REQUIRED_FIELDS]:
                raise Exception('Report file is missing required fields! %s, %s' % (html_path, rec))
...
def create_xml_records(paths, recursive):
    ...
    xml_path_iters = [get_filepaths(path, XML_FILENAME_PATTERN, recursive) for path in paths]
    ...
    for rec in xml_records:
        rec[REPORTTYPE] = 'xml' # JJ_TODO: Use a defined constant instead of string literal.
    return xml_records

def create_pdf_records(paths, recursive):
    ''' Find html and pdf files in paths and create ledger records for pdf reports based on parsed html data. '''
    # Search paths for HTML report files.
    html_path_iters = [get_filepaths(path, HTML_FILENAME_PATTERN, recursive) for path in paths]
    ...
    for rec in pdf_records:
        rec[REPORTTYPE] = 'pdf' # JJ_TODO: Use a defined constant instead of string literal.
    return pdf_records
...
def main():
    ...
    parser.add_argument('-p', '--publickey', metavar='PUBLIC_KEY_FILE', default=DEFAULT_PUBLIC_KEY_FILE, help='Filepath of RSA public key file (default: %s)' % DEFAULT_PUBLIC_KEY_FILE)
    parser.add_argument('paths', nargs='+', help='One or more paths of directories that will be processed when generating the ledger')
...
def run(paths, recursive=True, ledger_path=DEFAULT_LEDGER_FILE, privatekey_path=DEFAULT_PRIVATE_KEY_FILE, publickey_path=DEFAULT_PUBLIC_KEY_FILE):
